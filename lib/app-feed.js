...
function stampVersion(posts) {
  ...
  if (stamp.hash === hash && stamp.updatedAt) return stamp;

  const next = {
    dataVersion: Number(stamp.dataVersion || 0) + 1,
    hash,
...
export function buildFeed(collection, siteUrl) {
  ...
  const stamp = stampVersion(posts);
